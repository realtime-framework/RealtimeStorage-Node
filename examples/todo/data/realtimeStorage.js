...
RealtimeStorage.prototype = {
	initialize : function(){
		var configuration = { 			
			applicationKey: "2Ze1dz", // For security reasons change this to your application key
            authenticationToken: "TodoRealtimeStorage",            
            //privateKey : "8gpjgOwLR4uK",
            isSecure : true,
            url : "http://storage-backend-prd-useast1-s0001.realtime.co",
            isCluster : false,
		...
		Realtime.Storage.create(
			...
			function(ref) {
				this.storage = ref; 
				this.tableRef = this.storage.table("todoTable");
	...
	onPut : function(itemSnapshot){
		console.log("On put:",itemSnapshot);
		if(itemSnapshot){
	...
	onDelete : function(itemSnapshot){
		console.log("On delete:",itemSnapshot);
		var todo = itemSnapshot.val();
		todo.remove = true;
		if(this.notificationCallback != null) {
			this.notificationCallback(todo);		
	...
	onUpdate : function(itemSnapshot){
		console.log("On Update:",itemSnapshot);
		if(itemSnapshot){
			var todo = itemSnapshot.val();
			todo.state = todo.state == 1 ? true : false;
			if(this.notificationCallback != null) {
	...
	get: function (listName,callback) {
		if(!this.buffered(this.get,listName,callback)){	
			...
			this.tableRef = this.storage.table("todoTable").equals({item: "listName", value: listName });
...
exports = module.exports = RealtimeStorage.getInstance();